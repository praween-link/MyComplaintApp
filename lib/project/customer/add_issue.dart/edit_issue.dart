...
class CustomerEditComplaint extends StatefulWidget {
  static const routeName = '/customerEditComplaint';
  const CustomerEditComplaint({Key? key, required this.id}) : super(key: key);
  final String id;

  @override
  State<CustomerEditComplaint> createState() => _CustomerEditComplaintState();
}

class _CustomerEditComplaintState extends State<CustomerEditComplaint> {
  String type = '';
  String title = '';
  String category = '';
  String description = '';
  ...
  void addDataOfDB() {
    ...
    setState(() {
      type = customerController.issues[widget.id]!.type;
      title = customerController.issues[widget.id]?.title ?? '';
      category = customerController.issues[widget.id]?.category ?? '';
      description = customerController.issues[widget.id]?.description ?? '';
      name = customerController.issues[widget.id]?.name ?? '';
      phone = customerController.issues[widget.id]?.phone ?? '';
      pincode = customerController.issues[widget.id]?.pincode ?? '';
      state = customerController.issues[widget.id]?.state ?? '';
      city = customerController.issues[widget.id]?.city ?? '';
      area = customerController.issues[widget.id]?.area ?? '';
      locationTypeHome = customerController.issues[widget.id]?.locationtype ?? '';
    });
  }
        ...
        child: Padding(
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                // Text(
                //     '{t:${customerController.issues[widget.id]?.title ?? ''}, c:$category, d:$description, n:$name, p:$phone, pin:$pincode, s:$state, ci:$city, a:$area, lt:$locationTypeHome}'),
                ...
                GestureDetector(
                  onTap: () {
                    if (_formKey.currentState!.validate() && category != '') {
                      ...
                      customerController.UpdateIssueToDB(issue, widget.id);
                      AwesomeDialog(
                              context: context,
